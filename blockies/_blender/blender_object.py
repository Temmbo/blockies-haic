...
class BoneRotation:
    ...
    right: float = 0.0
    arm_right_top: float = 0.0
    arm_right_bottom: float = 0.0

    center_left: float = 0.0
    left: float = 0.0
    arm_left_top: float = 0.0
    arm_left_bottom: float = 0.0
...
class BlockiesBlenderObject:
    ...
    def _get_object_locations(self) -> Dict[str, Tuple[float, float, float]]:
        ...
        return {
            "arm_left_top": Vector((0.0, -1.5, 1.0)),
            "arm_left_bottom": Vector((0.0, -1.5, -1.0)),
            "spine_left": Vector((0.0, -1.0, 0)),
            "spine_left_center": Vector((0.0, 0.0, 0.0)),
            "spine_right_center": Vector((0.0, 1.0, 0.0)),
            "spine_right": Vector((0.0, 2.0, 0.0)),
            "arm_right_top": Vector((0.0, arm_pos, 1.0)),
            "arm_right_bottom": Vector((0.0, arm_pos, -1.0)),
        }
    ...
    def _create_model(self):
        ...
        for name, location in object_locations.items():
            bpy.ops.mesh.primitive_cube_add(
                size=self.cube_size, enter_editmode=True, location=location
            )
    ...
    def _create_armature(self):
        ...
        def add_bone(
            bone_name: str, from_block: str, to_block: str, forked: bool = False
        ):
            bpy.ops.armature.extrude_move(
                ARMATURE_OT_extrude={"forked": forked},
                TRANSFORM_OT_translate={"value": block_offset(to_block, from_block)},
            )
        ...
        add_bone("right", from_block="spine_right_center", to_block="spine_right")

        select("right", "tail")
        add_bone("arm_right_top", from_block="spine_right", to_block="arm_right_top")

        select("right", "tail")
        add_bone(
            "arm_right_bottom", from_block="spine_right", to_block="arm_right_bottom"
        )
        ...
        add_bone("left", from_block="spine_left_center", to_block="spine_left")

        select("left", "tail")
        add_bone("arm_left_top", from_block="spine_left", to_block="arm_left_top")

        select("left", "tail")
        add_bone("arm_left_bottom", from_block="spine_left", to_block="arm_left_bottom")
        attach_bones_to_blocks("left")

    def set_pose(self, bending: BoneRotation, bone_translation: BoneRotation = None):
    ...
    def create_bounding_box(self):
        ...
        bpy.ops.transform.resize(
            value=(max_x - min_x, max_y - min_y, max_z - min_z),
    ...
    def __init__(
        ...
        self.center()
        self._set_spherical(
            amount=main_spherical,
            amount2=sec_spherical,
