...
class BlockySampler:
  ...
  obj_name: Discrete = utils.discrete({"healthy": 0.25, "overbending":0.25, "bone_degen":0.25, "weak_spine":0.25})
  num_ill_chars: Discrete = dataclasses.field(
      default_factory=lambda: {
          "healthy": utils.discrete({0: 0.25, 1: 0.75}),
          #"ocd": discrete({2: 0.75, 3: 0.25}),
          "overbending": utils.discrete({2: 1.0}),
          "bone_degen": utils.discrete({2: 1.0}),
          "weak_spine": utils.discrete({2: 1.0}),
      }
  )
  ...
  def __post_init__(self):
      ...
      self.ill_chars = {
          ...
          "overbending": ("high_bend", "stretchy"),
          "bone_degen": ("mutation_mainbones", "high_sphere_diff"),
          "weak_spine": ("high_bend", "mutation_mainbones"),
      }
  ...
  def sample_ill_chars(
    ...
    #       params.ill_chars = ("high_bend", "mutation_mainbones")
      ...
      if obj_name == "healthy":
            ...
            ill_chars = tuple(char_sampler().tolist())
            params.ill_chars = ill_chars
      elif obj_name == "overbending":
            params.ill_chars = ("high_bend", "stretchy")
      elif obj_name == "bone_degen":
            params.ill_chars = ("mutation_mainbones", "high_sphere_diff")
      elif obj_name == "weak_spine":
            params.ill_chars = ("high_bend", "mutation_mainbones")
  ...
  def sample(self, obj_name: Optional[str] = None) -> SceneParameters:
    ...
    params.check_values()
    #print(params)
    #print out the sampling status of all attributes for debugging
    #for attr, status in params._attributes_status.items():
    #   print(f"{attr}: {status}")
    return params
