...
def test_generic_sampler():
    ...
    with pytest.raises(KeyError):
        blockies.BlockySampler._sample("ronny", test_dict)


def test_samplers_valid():
    """Test if the custom samplers run."""

    samplers = [blockies.BlockySampler(), blockies.BlockyHighVarSampler()]
...
def test_resampeling():
    ...
    sampler = blockies.BlockyHighVarSampler()
    param1 = blockies.SceneParameters()
    ...
    sampler.sample_main_spherical(param2)
    param3 = param2.clone()
    sampler.sample_arm_position(param3)
    assert param1.get_status("obj_rotation_roll") == "sampled"
    assert param2.get_status("main_spherical") == "resampled"
    assert param3.get_status("main_spherical") == "resampled"
...
def test_distribution_is_used():
    ...
    TODO: fix this test to deal with conditional distributions.
    """
